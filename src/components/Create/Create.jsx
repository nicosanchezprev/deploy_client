...
function validate(input) {
  ...
  if (input.types.length === 0) {
    errors.types = "Debe tener por lo menos un tipo y deben ser unicos";
  }
...
const Create = (props) => {
  ...
  const submitHandler = (event) => {
    ...
    if(!Object.keys(errors).length){
      const checkName = allPokes.find(p => p.name === input.name);
      if(!checkName) {
      ...
      alert("¡Todavia hay errores que debes evitar!");
    }
  };
  ...
  return (
    ...
    <div className="nav-detail">
     ...
     <div className="div-padre-form">
      <form className="form-create" onSubmit={(e) => submitHandler(e)}>
      ...
      <div className="div-hijo-form">
        ...
        <div className="form-mitad2">
          ...
          <div className="form-mitad3">
            <button className="boton-submit-create" type="submit" disabled={btnDisabled}>CREAR</button>
          </div>
        </div>
      </div>
      </form>
